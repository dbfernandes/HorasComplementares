...
class MonitoriaController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        $periodoAtivo = $this->getPeriodoAtivo();

        if ($periodoAtivo == null )
            return $this->render('index', ['erro' => 3]);

        //Verifica se as inscrições para monitoria estão abertas
        $periodoInscricao = $this->getInscricaoMonitoria();
        if ($periodoInscricao == null)
            return $this->render('index', ['erro' => 4]);
            //return $this->redirect(['index','erro3' => 'A inscrição para monitoria não está aberta.']);
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if (Yii::$app->request->post('step') == '1') {
                ...
                return $this->render('create', [
                    'model' => $model,
                    'etapa' => '2',
                    'periodo' => $periodoInscricao->codigo,
            ...
            $periodo = explode("/", $periodoInscricao->codigo);
            $searchModel->anoPeriodo = intval($periodo[0]);
            $searchModel->numPeriodo = intval($periodo[1]);
    ...
    public function actionAluno()
    {
        ...
        $dataProvider = $searchModel->searchAluno(Yii::$app->request->queryParams+['AlunoMonitoriaSearch' => ['=', 'periodo' => $periodoAtivo->codigo]]);
    ...
    public function actionSecretaria()
    {
        ...
        $searchModel = new AlunoMonitoriaSearch();
        $dataProvider = $searchModel->searchSecretaria(Yii::$app->request->queryParams+['AlunoMonitoriaSearch' => ['=', 'periodo' => $periodoAtivo->codigo]]);
    ...
    public function actionProfessor()
    {
        ...
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams+['ProfessorMonitoriaSearch' => ['=', 'periodo' => $periodoAtivo->codigo]]);
    ...
    public function actionAvaliador()
    {
        ...
        if ($comissao != null && $comissao->idProfessor != null) {
            ...
            $dataProvider = $searchModel->searchAvaliador(Yii::$app->request
                ->queryParams+['AlunoMonitoriaSearch' => ['=', 'periodo' => $periodoAtivo->codigo]]);
    ...
    public function actionJulgarinscricao($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) 
        {
            // Atualiza o status da monitoria
    ...
    public function actionFormularioinscricao($id)
    {
        ...
        // Cabeçalho
        $mpdf->WriteHTML('
            <img src="../web/images/cabecalho3.png" alt="Universidade Federal do Amazonas...." width="950" height="85">
    ...
    public function actionPlanosemestral($periodo)
    {
        //Observação: Justificativa é campo obrigatório do documento Plano Semestral de Monitoria(1)
        
        $arrayPeriodo = explode("/", $periodo);
        $searchModel = new DisciplinaMonitoriaSearch();
        $dataProvider = $searchModel->searchByPeriodo($arrayPeriodo[0], $arrayPeriodo[1]);

        $model = Periodo::find()->where(['codigo' => $periodo])->one();

        if ($model->load(Yii::$app->request->post()) && $model->save()) 
            return $this->actionGerarplanosemestral($model->id);
        else 
            return $this->render('planosemestral', 
                [
                    'searchModel' => $searchModel, 
                    'dataProvider' => $dataProvider, 
                    'model' => $model,
    ...
    {
        $modelPeriodo = Periodo::find()->where(['id' => $id])->one();
        ...
        if ( $modelPeriodo != null ) 
        {
            ...
            $mpdf->SetHTMLHeader('
                <img src="../web/images/cabecalho1.png" alt="Universidade Federal do Amazonas...." width="980" height="100">
                ');

            $mpdf->SetHTMLFooter('<p>&nbsp;</p><p>&nbsp;</p><p>&nbsp;</p><p>
                ...
                <table id="cabecalho1" width="99%" height="100%">
                 ...
                 <table id = "cabecalho2" width="21%" height="100%">
                    <tr>
                      ...
                      <td width="10px">'.$modelPeriodo->codigo.' </td> 
                    </tr> 
                 </table>
            ');
            ...
            $mpdf->WriteHTML('
                <br>
                <table id="table_header" width="1000px" height="5">
                    <tr>
            ...
            if ($disciplinas == null)
                return $this->render('index'); // Naum ha nenhuma disciplina com o período atual
            else 
            {
                $arrayPeriodo = explode("/", $modelPeriodo->codigo);
                $contador = 0;
                ...
                {                    
                    ...
                    $l1 = DisciplinaMonitoria::find()->where(['nomeDisciplina' => $disc->nomeDisciplina])
                                                    ...
                                                    ->andWhere(['anoPeriodo' => $arrayPeriodo[0]])
                                                    ->andWhere(['numPeriodo' => $arrayPeriodo[1]])
                                                    ->andWhere(['lab' => 1])
                                                    ->all();  
                    ...
                    $mpdf->WriteHTML('<table id="plano_semestral" width="1000px" height="5">');
            ...
            $mpdf->WriteHTML('
                ...
                '.$modelPeriodo->justificativaPlanoSemestral.''
            );   
    ...
    public function actionGerarquadrogeral($periodo)
    {
        $modelPeriodo = Periodo::find()->where(['codigo' => $periodo])->one();
                ...
                <table id="cabecalho_1_QuadroGeral" width="99%" height="100%">
                 ...
                 <table id = "cabecalho_2_QuadroGeral" width="40%" height="100%">
                    <tr>
                      ...
                      <td width="20px">'.$modelPeriodo->codigo.' </td> 
                    </tr> 
                 </table>
    ...
    public function actionFrequenciageral($periodo, $mes, $ano)
    {
        $modelPeriodo = Periodo::find()->where(['codigo' => $periodo])->one();
        ...
        { 
            ...
            $mpdf->SetHTMLHeader('
                    <img src="../web/images/cabecalho6.png" alt="Universidade Federal do Amazonas...." width="995" height="88" >
            ');
            $mpdf->SetHTMLFooter('<p style="text-align: center;">monitoriaufam@outlook.com</p>
            ');
                    ...
                    <table id="cabecalho_1_frequenciaGeral" width="99%" height="100%">
                     ...
                     <table id = "cabecalho_3_QuadroGeral" height="100%">
                        <tr>
                            ...
                            <td width="124px">'.$modelPeriodo->codigo.'</td>
            ...
            $alunos = AlunoMonitoria::find()->where(['periodo' => $modelPeriodo->codigo])
                                            ->andFilterWhere(['or', ['like','status', 'Selecionado com bolsa'], ['like','status', 'Selecionado sem bolsa']])
            ...
            {
                if (($n - $marcador) == 19)  // Esses if é para evitar que as linhas das tabelas passem por cima da imagem do header
                {
                    $mpdf->AddPage();
                    $mpdf->WriteHTML('
                        <br><br><br><br>
    ...
    public function actionFrequenciaindividual($idmonitoria, $mes, $ano)
    {
        $monitoria = AlunoMonitoria::find()->where(['id' => $idmonitoria])->one();

        if ($monitoria == null )
            return $this->render('index', ['erro' => 1]);
        else
        {
            // Usuário não pode acessar monitorias de outros usuários
            $aluno = Usuario::findOne(['CPF' => Yii::$app->user->identity->cpf]);
            if ($aluno->id != $monitoria->IDAluno)
                return $this->render('index', ['erro' => 5]);
        ...
        { 
            ...
            // Cabeçalho do doc
            $mpdf->SetHTMLHeader('
                <img src="../web/images/cabecalho5.png" alt="Universidade Federal do Amazonas...." width="980" height="100">
                ');
            ...
            $mpdf->WriteHTML('
                ...
                <table id="periodoLetivo_mesAno" width="56%">
                    <tr>
                        ...
                        <td width="21%">'.$monitoria->periodo.'</td>
            ...
            $dia = $ano .'-'.$mes.'-01';  // Monta a data do primeiro dia do mês
            $diaTotal = date('d', strtotime(date('Y-m-t', strtotime($dia)))); // Pega o último dia do mês
            ...
            while ($cont_dia <= $diaTotal) 
            {
                ...
                // Monta array da linha "CARGA HORÁRIA"
                ...
                $array_diaSemana[] = Monitoria::nomeDia(date('w', strtotime($dia)));  // Pega tradução do dia da semana e põe no array da linha "DIA DA SEMANA"
                $aux = strtotime("+1 day", strtotime($dia));
                $dia = date('Y-m-d', $aux);
                $array_diaMes[] = $cont_dia; // array da linha "DIA DO MES"
            ...
            if ( $diaTotal == 28) {
                $mpdf->WriteHTML('
                    <table id="calendario"  height="25px" width="900px">
                        <tr>
                            <td bgcolor="#e6e6e6" >
                            <img src="../web/images/dia_da_semana.png" alt="DIA DA SEMANA" width="42px" height="50px">
                            </td>
                            ...
                            <td width="30px">'.$array_diaSemana[27].'</td> <td width="30px"> -- </td> <td width="30px"> &nbsp; </td>
                            <td width="30px"> &nbsp; </td>
                        </tr>
                    </table>
                ');  
                ...
                $mpdf->WriteHTML('
                    <table id="calendario" height="25px" width="902px">
                        <tr>
                            <td bgcolor="#e6e6e6" >
                            <img src="../web/images/dia_do_mes.png" alt="DIA DO MÊS" width="42px" height="50px">
                            </td>
                            ...
                            <td width="30px">'.$array_diaMes[27].'</td> <td width="30px">29</td> <td width="30px">30</td>
                            <td width="30px">31</td>
                        </tr>
                    </table>
                ');
                ...
                $mpdf->WriteHTML('
                    <table id="calendario" height="25px" width="90px">
                        <tr>
                            <td bgcolor="#e6e6e6">
                            <img src="../web/images/carga_horaria.png" alt="CARGA HORÁRIA" width="42px" height="50px">
                            </td>
                ...
                $mpdf->WriteHTML('
                    <table id="calendario"  height="25px" width="900px">
                        <tr>
                            ...
                            <td width="30px">'.$array_diaSemana[21].'</td> <td width="30px">'.$array_diaSemana[22].'</td> <td width="30px">'.$array_diaSemana[23].'</td>
                            <td width="30px">'.$array_diaSemana[24].'</td> <td width="30px">'.$array_diaSemana[25].'</td> <td width="30px">'.$array_diaSemana[26].'</td>
                            <td width="30px">'.$array_diaSemana[27].'</td> <td width="30px">'.$array_diaSemana[28].'</td> <td width="30px"> &nbsp; </td>
                            <td width="30px"> &nbsp; </td>
                        </tr>
                    </table>
                ');  
                ...
                $mpdf->WriteHTML('
                    <table id="calendario" height="25px" width="902px">
                        <tr>
                            ...
                            <td width="30px">'.$array_diaMes[21].'</td> <td width="30px">'.$array_diaMes[22].'</td> <td width="30px">'.$array_diaMes[23].'</td>
                            <td width="30px">'.$array_diaMes[24].'</td> <td width="30px">'.$array_diaMes[25].'</td> <td width="30px">'.$array_diaMes[26].'</td>
                            <td width="30px">'.$array_diaMes[27].'</td> <td width="30px">'.$array_diaMes[28].'</td> <td width="30px">30</td>
                            <td width="30px">31</td>
                        </tr>
                    </table>
                ');
    ...
    public function getInscricaoMonitoria()
    {
        //Yii::$app->formatter->asDate(date('Y-m-d'), 'php:d-m-Y');
        $hoje = date('Y-m-d');
        $periodo = Periodo::find()->where(['and', 'dtInicioInscMonitoria <= "'.$hoje.'"', 'dtTerminoInscMonitoria >= "'.$hoje.'"'])->orderBy(['id' => SORT_DESC])->one();
        //$periodo = Periodo::find()->where(['and', 'dtInicioInscMonitoria <= "'.$hoje.'"', 'dtTerminoInscMonitoria >= "'.$hoje.'"'])->andWhere(['isAtivo' => 1])->orderBy(['id' => SORT_DESC])->one();
    ...
    public function actionModalfiltrorelatorio($funcao)
    {
        if (Yii::$app->request->post('periodo'))
        {
            if ($funcao == 'planosemestral')
                return $this->redirect(['planosemestral', 'periodo' => Yii::$app->request->post('periodo')]);
            else if ($funcao == 'gerarquadrogeral')
                return $this->redirect(['gerarquadrogeral', 'periodo' => Yii::$app->request->post('periodo')]);
            else if ($funcao == 'frequenciageral')
                return $this->redirect(['frequenciageral', 
        ...
        {
            $periodos = Periodo::find()->distinct()->orderBy(['codigo' => SORT_DESC])->asArray()->all();
            $arrayPeriodos = ArrayHelper::map($periodos, 'codigo', 'codigo');

            $cmd = Yii::$app->db->createCommand("SELECT distinct year(dmy)  as ano FROM frequencia ORDER BY year(dmy) DESC");
            //array com os anos de todas as frequências cadastradas
            $anos = $cmd->queryAll();
            $arrayAnos = ArrayHelper::map($anos, 'ano', 'ano');

            return $this->renderAjax('_form6', ['funcao' => $funcao, 'arrayPeriodos' => $arrayPeriodos, 'arrayAnos' => $arrayAnos]);
        }
    }
}
